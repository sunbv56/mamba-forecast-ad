...
from src.evaluation.metrics import calculate_threshold_pot, calculate_threshold_robust

def visualize_full_lifecycle(model_path, config_path, device, skip_first_pct=0.0, file_stride=1):
    """
    Vẽ xu hướng toàn bộ vòng đời của vòng bi (tất cả các file trong processed_dir).
    """
    # --- Toggles ---
    USE_PHYSICAL_STATS = True  
    ...
    # Model parameters (checking 'model' config first, then 'data', then defaults)
    patch_size = config['model'].get('patch_size', config['data'].get('patch_size', 2048))
    patch_stride = config['model'].get('patch_stride', config['data'].get('stride', 1024))
    
    # Dataset sliding window stride
    ...
    dataset = B02Dataset(processed_dir, lookback, horizon, window_stride, split='test',
    ...
    if file_stride > 1:
        print(f"Applying file stride of {file_stride}...")
        all_files = all_files[::file_stride]
    ...
    # Pre-compute highpass filter outside the loop
    b, a = None, None
    if highpass_freq > 0:
    ...
    with torch.no_grad():
        for i, f_name in enumerate(tqdm(all_files)):
            actual_idx = i * file_stride + start_idx
    ...
    # 3. Calculate Thresholds (POT and Robust)
    healthy_scores = [avg_errors[i] for i, idx in enumerate(file_indices) if skip_end_abs <= idx < train_end_abs]
    if len(healthy_scores) == 0:
        healthy_scores = avg_errors[:max(1, int(len(avg_errors) * 0.2))]
    
    thresh_pot = calculate_threshold_pot(healthy_scores, q=1e-3)
    thresh_robust = calculate_threshold_robust(healthy_scores, k=3)
    
    print(f"POT Threshold: {thresh_pot:.6f}")
    print(f"Robust Threshold: {thresh_robust:.6f}")

    # 4. Plot Combined Trends
    ...
    ax1_twin.plot(file_indices, avg_errors, color=color_err, linewidth=2, alpha=0.6, label='Anomaly Score')
    ax1_twin.axhline(y=thresh_pot, color='darkred', linestyle='--', alpha=0.8, label=f'POT Threshold ({thresh_pot:.4f})')
    ax1_twin.axhline(y=thresh_robust, color='purple', linestyle='-.', alpha=0.8, label=f'Robust Threshold ({thresh_robust:.4f})')
    ...
    ax3.axhline(y=3.0, color='gray', linestyle=':', alpha=0.5, label='Normal Kurtosis (3.0)')
    add_regions(ax3)

    ax3_twin = ax3.twinx()
    ax3_twin.set_ylabel(label_err, color=color_err, fontsize=12)
    ax3_twin.plot(file_indices, avg_errors, color=color_err, linewidth=1.5, alpha=0.4, label='Anomaly Score')
    ax3_twin.axhline(y=thresh_pot, color='darkred', linestyle='--', alpha=0.8, label=f'POT Threshold ({thresh_pot:.4f})')
    ax3_twin.axhline(y=thresh_robust, color='purple', linestyle='-.', alpha=0.8, label=f'Robust Threshold ({thresh_robust:.4f})')
    ...
    ax4_twin.plot(file_indices, avg_errors, color=color_err, linewidth=1.5, alpha=0.4, label='Anomaly Score')
    ax4_twin.axhline(y=thresh_pot, color='darkred', linestyle='--', alpha=0.8, label=f'POT Threshold ({thresh_pot:.4f})')
    ax4_twin.axhline(y=thresh_robust, color='purple', linestyle='-.', alpha=0.8, label=f'Robust Threshold ({thresh_robust:.4f})')
    ax4_twin.tick_params(axis='y', labelcolor=color_err)
    ax4_twin.set_yscale('log')

    # Combine legends for Subplot 3
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file_stride", type=int, default=1, help="Stride for files to evaluate (default: 1, set higher for faster evaluation)")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    
    args = parser.parse_args()
    
    visualize_full_lifecycle(args.model_path, args.config, args.device, skip_first_pct=args.skip_first_pct, file_stride=args.file_stride)
